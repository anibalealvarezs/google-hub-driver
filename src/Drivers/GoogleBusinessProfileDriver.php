...
class GoogleBusinessProfileDriver implements SyncDriverInterface, CanonicalMetricDictionaryProviderInterface, ChanneledAccountableInterface, LocationableInterface, MetricProfileProviderInterface, AggregationProfileProviderInterface
{
    ...
    {
        $calculateSynthetics = false;

        try {
            $chanConfig = DriverInitializer::validateConfig($this->getChannel());
            if (isset($chanConfig['granular_sync'])) {
                $calculateSynthetics = filter_var($chanConfig['granular_sync'], FILTER_VALIDATE_BOOLEAN);
            }
        } catch (\Exception $e) {
        }

        return $calculateSynthetics ? InstanceTier::POWERED : InstanceTier::TIER_2;
    }
